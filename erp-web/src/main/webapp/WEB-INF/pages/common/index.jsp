...
<script type="text/javascript">
	...
	$(function() {
		...
		// 系统管理菜单加载
		$.ajax({
			url : '${pageContext.request.contextPath}/functionAction_findMenuForSystem.action',
			type : 'POST',
			dataType : 'text',
			success : function(data) {
				var zNodes = eval("(" + data + ")");
		...
		window.setTimeout(function(){
			$.messager.show({
				title:"消息提示",
				msg:'欢迎登录，${loginEm.name}！ <a href="javascript:void" onclick="top.showAbout();">联系管理员</a>',
				timeout:5000
			});
		},3000);
		/*************/
	...
	function logoutFun() {
		...
		.confirm('系统提示','您确定要退出本次登录吗?',function(isConfirm) {
			if (isConfirm) {
				location.href = '${pageContext.request.contextPath }/empAction_logout.action';
			}
		});
	}
	// 修改密码
	...
	// 版权信息
	function showAbout(){
		$.messager.alert("ERP v1.0","管理员邮箱: zqx@itcast.cn");
	}
</script>
</head>
<body class="easyui-layout">
	<div data-options="region:'north',border:false"
		...
		<div id="sessionInfoDiv"
			...
			[<strong>${loginEm.username}</strong>]，欢迎你！
		</div>
				...
				<tr>
					<td style="width: 300px;">
						<div style="color: #999; font-size: 8pt;">
							AHPU | Powered by <a href="javascript:void(0)">duanle</a>
						</div>
					</td>
        ...
        <div class="easyui-layout" fit="true">
            <div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
                ...
                <table cellpadding=3>
                    <tr>
                        <td>新密码：</td>
                        <td><input required="true" data-options="validType:'length[6,12]'" id="txtNewPass" type="Password" class="txt01 easyui-validatebox" /></td>
                    </tr>
                    <tr>
                        <td>确认密码：</td>
                        <td><input required="true" data-options="validType:'length[6,12]'" id="txtRePass" type="Password" class="txt01 easyui-validatebox" /></td>
                    </tr>
                </table>
                </form>
            </div>
